...
from LayerBank import ConvLayer, DenseLayer, FractionallyStridedConvLayer
...
# Globalne (ne)promenljive - Hiperparametri modela
...
class DCGAN:
  def __init__(self, img_length, num_colors, d_sizes, g_sizes):
    ...
    # save for later
    self.img_length = img_length #img_legnth se odnosi i na height i na width. Pretpostavljamo da je slika predstavljena kao kvadratna matrica
    ...
    """
    S obzirom na to da obucavamo dve neuralne mreze paralelno, potrebno je da naglasimo
    koji optimizator azurira koji skup parametara. S toga prvo preuzimamo sve parametre od
    diskriminatora (naziv im pocenje uvek sa 'd'), pa onda preuzimamo sve parametre od
    generatora (naziv im pocinje uvek sa 'g') nakon cega vrsimo propagaciju gresaka unazad
    koristeci se odgovarajucim optimizatorima za svaki od ova dva skupa parametara, kako
    bismo znali kako da azuriramo jednu mrezu a kako drugu.
    """
  ...
  def build_discriminator(self, X, d_sizes):
    """
    Pozivom ove funkcije automatski definisemo opseg "discriminator",
    sto znaci sve sto budemo definisali u okviru ove funkcije ce biti
    definisiano i dostupno u okviru ovog opsega
    """
    with tf.variable_scope("discriminator") as scope:

      self.d_convlayers = []
      num_of_input_channels = self.num_colors
      dim = self.img_length
      count = 0
      for num_of_filters, filter_size, stride, apply_batch_norm in d_sizes['conv_layers']:
        name = "convlayer_%s" % count
        count += 1

        layer = ConvLayer(name, num_of_input_channels, num_of_filters, apply_batch_norm, filter_size, stride, lrelu)
        self.d_convlayers.append(layer)
        num_of_input_channels = num_of_filters
        print("dim:", dim)
